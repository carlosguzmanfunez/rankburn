...
export type RefundRequest = {
  /** our own payment id, reused as the provider idempotency key */
  paymentId: string
  providerCaptureId: string
  amountCents: number
  currency: string
...
export type PaymentProvider = {
   ...
   *
   * Must be safe to call more than once for the same `paymentId`: the provider
   * idempotency key plus our own `refunded_at` guard together ensure one
   * payment can never produce two refunds.
   */
  refundCapture(request: RefundRequest): Promise<RefundResult>
  /**
