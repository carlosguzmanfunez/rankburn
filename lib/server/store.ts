...
export type CampaignStats = {
  ...
  peakRank: number
  /** Raw stored value. Authoritative; never reconstruct it from hours. */
  minutesAtOne: number
  ...
  /** Derived for display only. Lossy by design: whole hours. */
  hoursAtOne: number
  joinedLabel: string
  rankHistory: number[]
}
...
/**
 * Seeds and local development ONLY.
 *
 * No product route may assume this is the authenticated user. Migration 0009
 * nulls its password hash, so no credential path can ever authenticate as it.
 */
...
export function mapCampaign(row: CampaignRow): Campaign {
  return {
    ...
    previousCampaignId: row.previousCampaignId ?? undefined,
    ...
    burnRateChangedAt: new Date(row.burnRateChangedAt).toISOString(),
    balanceChangedAt: new Date(row.balanceChangedAt).toISOString(),
  }
}

export function mapProduct(row: ProductRow): Product {
  // Normalised, never cast. A row written before the taxonomy was unified
  // still carries a legacy id if migration 0006 has not been applied to that
  // database yet; coercing here means nothing downstream can receive a
  // category outside the union.
  ...
  return {
    ...
    category,
    subtype: normalizeSubtypeId(category, row.subtype, row.category),
    website: row.website,
    hue: row.hue,
    createdAt: new Date(row.createdAt).toISOString(),
  }
...
export function mapStats(row: StatsRow): CampaignStats {
  return {
    ...
    // Sampled in minutes; hours are a lossy display projection of the same
    // value. Anything that WRITES the counter must use `minutesAtOne`.
    minutesAtOne: row.minutesAtOne,
