...
export async function applyModeration(
  ...
  return db.transaction(async (tx) => {
     ...
     * Typed on purpose.
     *
     * This used to be a `Record<string, unknown>`, which IS assignable to
     * Drizzle's all-optional update type - so neither the compiler nor the
     * schema audit ever checked these column names. A typed literal restores
     * both.
     */
    type CampaignPatch = Partial<typeof campaigns.$inferInsert>
    ...
    switch (action) {
      case 'APPROVE': {
        ...
        // Approval clears moderation, it does not fund anything. With no
        // balance the campaign is APPROVED and awaiting verified payment -
        // NOT exhausted. Marking it EXHAUSTED would strand it in a terminal
        // state before it ever ran, which is exactly the damage migration
        // 0003 exists to repair.
        nextStatus = hasBudget ? 'ACTIVE' : 'APPROVED'
      ...
      case 'PAUSE': {
        // A run that never started and a run that already ended cannot be
        // "temporarily removed from the market". Allowing EXHAUSTED -> PAUSED
        // would also let a finished run leave its terminal state.
        if (current !== 'ACTIVE') {
          return {
            ok: false as const,
